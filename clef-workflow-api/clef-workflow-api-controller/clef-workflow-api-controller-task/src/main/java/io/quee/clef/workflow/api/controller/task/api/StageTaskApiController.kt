...
class StageTaskApiController(private val clefWorkflowEngine: ClefWorkflowEngine) : StageTaskApiContract {
    ...
    override fun details(key: String): ContractResponse<TaskDetailsResponse> {
        return clefWorkflowEngine.stageTaskContract.details(key)
    }

    override fun enable(key: String): ContractResponse<SharedResponse> {
        return clefWorkflowEngine.stageTaskContract.enable(key)
    }

    override fun disable(key: String): ContractResponse<SharedResponse> {
        return clefWorkflowEngine.stageTaskContract.disable(key)
    }

    override fun delete(key: String): ContractResponse<SharedResponse> {
        return clefWorkflowEngine.stageTaskContract.delete(key)
    }
}